...
| Host | **GitHub Pages** (Actions-based deploy). Nitro preset `github_pages`. Build stays host-agnostic: base URL + site URL come from env, no host logic in app code |
...
Contrast audit (computed): `#5C6470` = 3.25:1 on bg → **fails AA** for text (used ~12x in design). `#343A44` is border only (fine, non-text). Fix: token `faint` lifted to ≈`#7A8391` (≥4.5:1) or restricted to decorative. Light palette (derived): bg `#F7F8FA`, surface `#FFFFFF`, surface-2 `#EEF0F3`, text `#14171C`, text-2 `#3D4450`, muted `#5B6472` (5.6:1), accent `#0F766E` (5.2:1; teal `#5EEAD4` unusable on light).
...
app/
  ...
  components/
    ...
    ui/      TagChip, ExternalLink, SectionHeading
  composables/ useProjectFilter, useProjects, useLocalizedProject
  data/      projects.ts, experience.ts, stack.ts, site.ts (social links, email)
  pages/     index.vue, projects/index.vue, projects/[slug].vue
  types/     content.ts
i18n/locales/ en.json, pl.json, i18n.config.ts
public/      cv.pdf, videos/, images/, favicon
scripts/     check-placeholders.mjs, check-locales.mjs (also unit test)
tests/       unit/, e2e/
.github/workflows/ ci.yml (reusable, `workflow_call` + PR trigger), deploy.yml
nuxt.config.ts, tailwind.config.ts (or v4 css), eslint.config.mjs, playwright.config.ts, vitest.config.ts
docs/        design/ (extracted tokens + boards), content-checklist.md
PRD.md
```
### 6.2 Data model (TS)
```ts
...
interface Project {
  ...
  pipeline?: { ingest: string[]; query: string[] } // keys -> i18n
...
`Experience {id, from, to|null, current}` + i18n `experience.<id>.{role,company,location,bullets[]}`. `StackGroup {id, items[]}`.
...
Env: `NUXT_PUBLIC_SITE_URL` (canonical/sitemap/OG), `NUXT_APP_BASE_URL` (subpath hosting). Local default `/` + placeholder domain.
...
- Project repo (`user.github.io/<repo>/`): `NUXT_APP_BASE_URL=/<repo>/`, `NUXT_PUBLIC_SITE_URL=https://user.github.io/<repo>`. User-site repo (`user.github.io`) or custom domain: base `/`.
...
### 6.6 SEO
`useSeoMeta` per page from i18n; nuxt-og-image component template (satori, Geist font, dark theme card, title/subtitle) per page × locale; `schema-org` Person on home; sitemap with i18n alternates; robots allow all (+ sitemap URL).
...
3. Add modules; `nuxt.config.ts` (ssr true, `nitro.preset: 'static'`, prerender, i18n, colorMode, fonts, image, site url/base env; preset chosen via `NITRO_PRESET`, not hardcoded).
4. ESLint (`@nuxt/eslint`), vue-tsc script, Husky + lint-staged pre-commit (eslint --fix + vue-tsc), Prettier optional.
...
24. `.github/workflows/deploy.yml` (GitHub Pages):
    ...
    jobs:
      ...
      build:
        ...
        steps:
          ...
          - run: node scripts/check-placeholders.mjs   # warn now, --strict once content is final
    ...
    ```
    One-time repo setup: Settings → Pages → Source = **GitHub Actions**; repo variables `SITE_URL` (+ `BASE_URL` only for custom domain = `/`). Add `.nvmrc`.
...
- [ ] Criteria in §3 met with evidence (LHCI report, axe report)
- [ ] CI green (lint, types, unit, e2e, lhci)
- [ ] Both themes reviewed vs design.html boards (desktop/mobile/detail)
- [ ] Content checklist complete; placeholder gate passes
- [ ] cv.pdf, photo, OG, favicon in place
...
- Light-theme review by user after derivation.
